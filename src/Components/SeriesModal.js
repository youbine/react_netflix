...
import { makeImagePath, certificationRatings } from "../helper";
...
function SeriesModal({ data, title }) {
  const navigate = useNavigate();
  const { scrollY } = useViewportScroll();
  const scrollYset = useTransform(scrollY, (value) => value + 50);
  const [searchParams, setSearchParams] = useSearchParams();
  const id = searchParams.get("id");
  const seriesClicked = id && data.find((series) => String(series.id) === id);

  const onOverlayClicked = () => {
    navigate(-1);
  };

  const { data: keywords } = useQuery(["series", "keyword", id], () =>
    getSeriesKeywords(`${id}`)
  );
  const { data: details } = useQuery(["series", "details", id], () =>
    getSeriesDetails(`${id}`)
  );
  const { data: certification } = useQuery(
    ["series", "certification", id],
  ...
  const { data: credits } = useQuery(["tv", "credits", id], () =>
    getCredits("tv", `${id}`)
  );
              ...
              <ModalCover>
                <ModalPicture
                  style={{
                    backgroundImage: `linear-gradient(rgba(0, 0, 0, 0), #141414), url(${makeImagePath(
                      seriesClicked.backdrop_path,
                      "original"
                    )})`,
                  }}
                />
                <ModalTitle>{seriesClicked.name}</ModalTitle>
                <Buttons
                  style={{
                    alignItems: "center",
                ...
                <ModalInfo>
                  <MaturityNRuntime style={{ height: "0%" }}>
                    ...
                    <span style={{ fontSize: "1rem" }}>{seasons} Seasons</span>
                  </MaturityNRuntime>
                  <Overview
                    style={{
                      margin: "0 auto",
